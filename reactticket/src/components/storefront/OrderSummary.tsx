...
export const OrderSummary = () => {
  ...
  return (
    <div
      style={{ marginTop: '20px', padding: '20px', borderTop: '1px solid #e2e8f0' }}
      role="region"
      aria-label={t('store.order.summary')}
      aria-live="polite"
    >
      <h3 style={{ margin: '0 0 10px 0' }}>{t('store.order.summary')}</h3>
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <span>{t('store.cart.subtotal')}:</span>{' '}
        <span>{formatCurrency(totals.subtotalCents, currency, locale)}</span>
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <span>{t('store.cart.discount')}:</span>{' '}
        <span>-{formatCurrency(totals.discountCents, currency, locale)}</span>
      </div>
      <div
        style={{
          display: 'flex',
      ...
      >
        ...
        <span>{formatCurrency(totals.totalCents, currency, locale)}</span>
      </div>
    </div>
  );
};
