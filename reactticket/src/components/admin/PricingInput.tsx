...
export const PricingInput: React.FC<PricingInputProps> = ({
  ...
  const getBreakdown = () => {
    ...
    if (mode === 'organizer') {
      ...
      const O = C * MULTIPLIER - FLAT_FEE_CENTS / 100;
      const formattedO =
        O > 0 ? formatCurrency(Math.round(O * 100), currency) : formatCurrency(0, currency);
  ...
  return (
    <div
      ...
      role="group"
      aria-label="Ticket Pricing Input"
    >
      ...
      <div
        style={{ fontSize: '11px', color: '#64748b', maxWidth: '300px', lineHeight: '1.4' }}
        aria-live="polite"
      >
        {getBreakdown()}
      </div>
