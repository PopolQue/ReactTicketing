        ...
        <div key={batch.id} style={{ marginBottom: '25px', border: '1px solid #e2e8f0', padding: '20px', borderRadius: '12px', background: 'white', opacity: batch.archived ? 0.6 : 1 }}>
          ...
          >
            ...
            <div onClick={(e) => e.stopPropagation()} style={{display: 'flex', gap: '10px'}}>
                <button type="button" style={{ padding: '2px 6px', fontSize: '10px' }} onClick={() => exportCSV(batch)} aria-label={`Export CSV for ${batch.name}`}>
                    Export CSV
                </button>
                <button type="button" style={{ padding: '2px 6px', fontSize: '10px' }} onClick={() => toggleBatchActive(batch.id)} aria-label={`${batch.codes.every(c => !c.active) ? 'Reactivate' : 'Deprecate'} ${batch.name}`}>
                    {batch.codes.every(c => !c.active) ? 'Reactivate' : 'Deprecate'}
                </button>
                <button type="button" style={{ padding: '2px 6px', fontSize: '10px' }} onClick={() => toggleBatchArchive(batch.id)} aria-label={`${batch.archived ? 'Unarchive' : 'Archive'} ${batch.name}`}>
          ...
          {expandedBatches.has(batch.id) && (
            <table style={{ width: '100%', borderCollapse: 'collapse' }} aria-label={`Promo codes for batch ${batch.name}`}>
              <thead>
              ...
              <tbody>
                {batch.codes.map(code => (
                  <tr key={code.code} style={{ borderBottom: '1px solid #f1f5f9' }}>
                    ...
                    <td style={{ padding: '8px' }}>
                      {!code.sentAt && <button type="button" style={{ padding: '4px 8px', fontSize: '11px', cursor: 'pointer' }} onClick={() => markAsSent(batch.id, code.code)} aria-label={`Mark code ${code.code} as sent`}>Mark Sent</button>}
                      <button type="button" style={{ padding: '4px 8px', fontSize: '11px', cursor: 'pointer', marginLeft: '5px' }} onClick={() => toggleCodeActive(batch.id, code.code)} aria-label={`${code.active ? 'Deprecate' : 'Activate'} code ${code.code}`}>
