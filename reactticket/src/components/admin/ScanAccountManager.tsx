  ...
  return (
    <section style={{ marginTop: '20px' }} role="region" aria-label="Scan Account Manager">
      ...
      <div
        ...
        role="group"
        aria-label="Create Scan Account"
      >
        <input
            ...
            <tr key={account.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
              <td style={{ padding: '8px' }}>
                {editingId === account.id ? (
                  <input
                    value={editValues.username ?? account.username}
                    onChange={(e) => setEditValues({ ...editValues, username: e.target.value })}
                    aria-label={`Edit username for ${account.username}`}
                  />
                ) : (
                  account.username
                )}
              ...
              <td style={{ padding: '8px' }}>
                {editingId === account.id ? (
                  <input
                    value={editValues.assignedLocation ?? account.assignedLocation}
                    onChange={(e) =>
                      setEditValues({ ...editValues, assignedLocation: e.target.value })
                    }
                    aria-label={`Edit assigned location for ${account.username}`}
                  />
                ) : (
              ...
              <td style={{ padding: '8px' }}>
                <span style={{ color: account.active ? 'green' : 'red', fontWeight: 'bold' }}>
                ...
                {editingId === account.id ? (
                  <>
                    <button
                      ...
                      aria-label={`Save changes for ${account.username}`}
                    >
                      Save
                    </button>
                    <button
                      ...
                      aria-label={`Cancel editing for ${account.username}`}
                    >
                      Cancel
                    </button>
                  </>
                  ...
                  <>
                    ...
                    <button
                      ...
                      aria-label={`${account.active ? 'Deactivate' : 'Reactivate'} account ${account.username}`}
                    >
                    ...
                    <button
                      ...
                      aria-label={`Reset PIN for account ${account.username}`}
                    >
                      Reset PIN
                    </button>
                    <button
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
              <button type="button" onClick={() => setResettingAccount(null)}>
                Cancel
              </button>
              <button
                type="button"
