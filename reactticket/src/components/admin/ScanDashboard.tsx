...
export const ScanDashboard: React.FC = () => {
  ...
  if (isLoading && !summary)
    return (
      <p role="status" aria-busy="true" aria-label="Loading analytics...">
        Loading analytics...
      </p>
    );
  if (error)
  ...
  return (
    <div
      className="scan-dashboard"
      style={{ marginTop: '20px' }}
      role="region"
      aria-label="Live Scan Dashboard"
    >
      ...
      >
        <div
          ...
          role="status"
          aria-label="Admitted / Issued stats"
        >
          <span style={{ fontSize: '12px', color: '#64748b' }}>Admitted / Issued</span>
          <div style={{ fontSize: '24px', fontWeight: 'bold' }}>
            {summary.totalAdmitted} / {summary.totalIssued}
          </div>
          <small style={{ color: '#22c55e' }}>
            {((summary.totalAdmitted / (summary.totalIssued || 1)) * 100).toFixed(1)}%
          </small>
        </div>
        <div
          className="stat-card"
          ...
          role="status"
          aria-label="Duplicates / Invalid stats"
        >
          <span style={{ fontSize: '12px', color: '#64748b' }}>Duplicates / Invalid</span>
          <div style={{ fontSize: '24px', fontWeight: 'bold' }}>
            {summary.duplicateScanCount} / {summary.invalidScanCount}
          </div>
      ...
      <div
        ...
        role="img"
        aria-label="Scan Velocity Chart"
      >
        {summary.scanVelocity.length === 0 ? (
          <p style={{ width: '100%', textAlign: 'center', color: '#64748b' }}>No recent activity</p>
        ) : (
          summary.scanVelocity.map((v, i) => (
            <div
              key={i}
              title={`${v.count} scans at ${v.timestamp.toLocaleTimeString()}`}
              style={{
                flex: 1,
                background: '#3b82f6',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
        <section>
          ...
          <table
            ...
            aria-label="Admission Rate by Ticket Type"
          >
            <tbody>
      ...
      {summary.clockSkewAnomalies > 0 && (
        <div
          ...
          role="alert"
        >
          <strong>Warning:</strong> {summary.clockSkewAnomalies} clock skew anomalies detected.
          Ensure crew devices have correct time settings.
        </div>
      )}
    </div>
  );
