...
export const ScanResult: React.FC<ScanResultProps> = ({ result, onDismiss }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      gainNode.gain.exponentialRampToValueAtTime(
        0.00001,
        audioContext.currentTime + (result.result === 'admitted' ? 0.15 : 0.4)
      );
    } catch (e) {
  ...
  return (
    ...
    >
      <h2
        style={{ fontSize: '3rem', margin: '0 0 10px 0', textShadow: '0 0 10px rgba(0,0,0,0.5)' }}
      >
      ...
      {ticket && (
        ...
        >
          ...
          <p style={{ margin: '5px 0' }}>
            Buyer:{' '}
            {typeof ticket.personalization?.name === 'string'
              ? ticket.personalization.name
              : 'Unknown'}{' '}
            {typeof ticket.personalization?.surname === 'string'
              ? ticket.personalization.surname
              : ''}
          </p>
        </div>
      )}
