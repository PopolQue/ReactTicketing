...
export default function VenueSelector({
  ...
  useEffect(() => {
    if (selectedVenueId && !selectedVenue) {
      supabase.from('venues').select('*').eq('id', selectedVenueId).single().then(({
        data
      }) => {
        if (data) {
  ...
  const {
    results: venues,
    loading
  } = useDebouncedSearch<Venue>(search, 'venues', 'name', 'is_verified', 300, skipCondition);
  useEffect(() => {
  ...
  const handleCreateNew = async () => {
    ...
    const {
      error
    } = await supabase.from('venues').insert([newVenueData]);
    if (!error) {
  ...
  return <div style={{
    position: 'relative'
    ...
    }} onBlur={() => setTimeout(() => setIsDropdownOpen(false), 200)} className="input-field" placeholder={t("searchForAVenueOrTypeTo")} required />

      {isDropdownOpen && search && <div style={{
      position: 'absolute',
      top: '100%',
      left: 0,
      right: 0,
      ...
      overflowY: 'auto'
        ...
        return <div key={venue.id} onClick={() => handleSelectVenue(venue)} style={{
          padding: '12px',
          ...
          justifyContent: 'space-between'
              ...
              <span>{venue.name} <span style={{
              color: 'var(--text-secondary)',
              fontSize: '0.85rem'
            }}>{venue.city ? `- ${venue.city}` : ''}</span></span>
              {venue.is_verified && <span style={{
            color: '#10b981',
            fontSize: '0.85rem'
          }}>{t("Verified")}</span>}
            </div>;
      })}
          {!loading && !venues.find(v => v.name.toLowerCase() === search.toLowerCase()) && <div onClick={handleCreateNew} style={{
        padding: '12px',
        color: 'var(--accent)',
        cursor: 'pointer',
        fontWeight: 500
      }} onMouseOver={e => e.currentTarget.style.backgroundColor = 'rgba(255,255,255,0.05)'} onMouseOut={e => e.currentTarget.style.backgroundColor = 'transparent'}>{t("Create")}{search}{t("AsANewVenue")}</div>}
        </div>}
    </div>;
}