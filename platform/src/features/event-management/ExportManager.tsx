...
export default function ExportManager({
  ...
  const exportToCSV = async () => {
    ...
    try {
      ...
      const rows = tickets.map((t: any) => [t.id, t.buyer_email, t.ticket_types?.name || 'Unknown', t.status.toUpperCase(), (t.price_paid_cents / 100).toFixed(2), new Date(t.issued_at).toLocaleString()]);
  ...
  return <div className="glass-panel" style={{
    padding: '24px'
      ...
      marginBottom: '16px',
      fontSize: '0.9rem'
    }}>{t("exportYourAttendeeListToU")}</p>
      <button onClick={exportToCSV} disabled={loading} className="btn-primary" style={{
      width: '100%',
      display: 'flex',
