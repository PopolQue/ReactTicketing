...
export default function LineupManager({ eventId, availableArtists, eventArtists, setEventArtists }: { eventId: string, availableArtists: any[], eventArtists: any[], setEventArtists: any }) {
  ...
  const handleAddArtist = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('event_artists').insert([
      { event_id: eventId, artist_id: selectedArtistId }
    ]);
    
    if (!error) {
      const addedArtist = availableArtists.find(a => a.id === selectedArtistId);
      if (addedArtist) {
        setEventArtists([...eventArtists, { event_id: eventId, artist_id: selectedArtistId, artists: addedArtist }]);
      }
      setSelectedArtistId('');
    } else {
  ...
  const removeArtist = async (artistId: string) => {
    const { error } = await supabase.from('event_artists').delete().match({ event_id: eventId, artist_id: artistId });
    if (!error) {
