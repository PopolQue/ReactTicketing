...
export default function TicketPersonalizationForm({
  ...
  return (
    <div style={{ marginBottom: '24px', padding: '20px', backgroundColor: 'rgba(0,0,0,0.3)', borderRadius: '8px' }}>
      <h4 style={{ margin: '0 0 16px 0', color: 'var(--accent)' }}>Ticket {index + 1}: {ticketForm.tier.name}</h4>
          ...
          {checkoutFields.map(f => (
            <div key={f.id}>
              ...
              <input 
                ...
                value={ticketForm.answers[f.id] || ''}
                onChange={e => onAnswerChange(ticketForm.id, f.id, e.target.value)}
