    ...
    async function fetchFriends() {
        ...
        if (data) {
            const friendsList = (data as unknown as Friendship[]).map(f => {
                if (f.user_id === user.id) return { id: f.id, userId: f.profile2?.id || '', username: f.profile2?.username || '' };
    ...
    return (
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h1>My Friends</h1>
                <button className="btn-primary" onClick={() => setIsModalOpen(true)}>+</button>
            </div>
            ...
            <h2>Friends List</h2>
            {friends.map(f => (
                <div key={f.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', margin: '10px 0' }}>
                    <button 
                    ...
                    <button onClick={(e) => unfriend(e, f.id)} className="btn-secondary" style={{ padding: '8px 12px', borderColor: '#ef4444', color: '#ef4444' }}>
                        {t('nav.unfriend') || 'Unfriend'}
                    </button>
                </div>
            ))}

            <AddFriendModal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); fetchRequests(); }} />
        </div>
    );
}
