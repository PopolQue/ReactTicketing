...
export default function PromosFullPage() {
  ...
  const batchesMap = promos.reduce(
    (acc, promo) => {
      if (promo.batch_id) {
  ...
  if (loading)
    return (
      <div style={{ padding: '40px', color: 'var(--text-secondary)' }}>
        {t('organizer.promos.loading')}
      </div>
    );

  return (
    <div
      className="manage-event-page"
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          <div className="glass-panel" style={{ padding: '24px' }}>
            <h3 style={{ margin: '0 0 16px 0', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <Plus size={18} /> {t('organizer.promos.createSingle')}
            </h3>
            <form
            ...
            >
              <input
                required
                type="text"
                placeholder={t('organizer.promos.codePlaceholder')}
                className="input-field"
                value={singleForm.code}
                onChange={(e) =>
                  setSingleForm({ ...singleForm, code: e.target.value.toUpperCase() })
                }
              />
              ...
              {singleForm.discount_kind !== 'free' && (
                <input
                  required
                  type="number"
                  placeholder={t('organizer.promos.valuePlaceholder')}
                  className="input-field"
                  value={singleForm.discount_value}
                  onChange={(e) => setSingleForm({ ...singleForm, discount_value: e.target.value })}
                />
              )}
          ...
          <div
            ...
            style={{ padding: '24px', border: '1px solid rgba(52, 96, 64, 0.3)' }}
          >
            <h3 style={{ margin: '0 0 16px 0', color: 'var(--accent)' }}>
              {t('organizer.promos.generateBatch')}
            </h3>
            <p
              style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '16px' }}
            >
              {t('organizer.promos.batchDesc')}
            </p>
            <form
            ...
            >
              <div style={{ display: 'flex', gap: '12px' }}>
                <input
                  type="text"
                  placeholder={t('organizer.promos.prefixPlaceholder')}
                  className="input-field"
              ...
              {batchForm.discount_kind !== 'free' && (
                <input
                  ...
                  placeholder={t('organizer.promos.valuePlaceholder')}
                  className="input-field"
                  value={batchForm.discount_value}
                  onChange={(e) => setBatchForm({ ...batchForm, discount_value: e.target.value })}
                />
              )}
              <button type="submit" className="btn-primary" disabled={generatingBatch}>
                {generatingBatch
                  ? t('organizer.promos.generating')
                  : `${t('organizer.promos.generateCodes')} ${batchForm.count}`}
              </button>
            </form>
          </div>
        </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', maxHeight: '600px' }}>
            {activeTab === 'individual' && (
              <>
                ...
                >
                  <span>
                    {promos.length} {t('organizer.promos.totalCodes')}
                  </span>
                  <button
                    onClick={() => exportCsv(promos, 'all_promos')}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {promos.map((p: any) => (
                    ...
                    >
                      <div>
                        <strong
                          style={{ display: 'block', marginBottom: '4px', letterSpacing: '1px' }}
                        >
                          {p.code}
                        </strong>
                        <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                          {p.discount_kind === 'free'
                            ? 'Free Ticket'
                            : p.discount_kind === 'percent_off'
                              ? `${p.discount_value}% Off`
                              : `€${(p.discount_value / 100).toFixed(2)} Off`}
                          {' | '}Uses: {p.used_count}/{p.max_uses || '∞'} |{' '}
                          {p.active ? (
                            <span style={{ color: '#10b981' }}>{t('organizer.promos.active')}</span>
                          ) : (
                            <span style={{ color: '#ef4444' }}>
                              {t('organizer.promos.inactive')}
                            </span>
                          )}
                        </div>
                      </div>
            ...
            {activeTab === 'batches' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                {sortedBatches.map(([batchId, batchCodes]) => {
                  ...
                  const displayPrefix = prefixMatch
                    ? prefixMatch[1]
                    : firstCode.code.substring(0, 4);
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <div>
                          <strong style={{ fontSize: '1.1rem', letterSpacing: '1px' }}>
                            {t('organizer.promos.batch')} {displayPrefix}***
                          </strong>
                          <div
                            style={{
                              fontSize: '0.85rem',
                          ...
                          >
                            {batchCodes.length} codes | {t('organizer.promos.generatedOn')}{' '}
                            {new Date(firstCode.created_at).toLocaleDateString()}
                          </div>
                          <div
                            style={{
                              fontSize: '0.85rem',
                          ...
                          >
                            {firstCode.discount_kind === 'free'
                              ? 'Free Ticket'
                              : firstCode.discount_kind === 'percent_off'
                                ? `${firstCode.discount_value}% Off`
                                : `€${(firstCode.discount_value / 100).toFixed(2)} Off`}
                          </div>
                        </div>
                        <button
                          onClick={() => exportCsv(batchCodes, `batch_${batchId.substring(0, 8)}`)}
