...
export default function ArtistDashboard() {
  ...
  async function fetchData() {
    ...
    // Fetch Analytics
    const { data: analyticsData } = await supabase.rpc('get_artist_analytics', {
      artist_id_param: activeEntity.id,
    });
        ...
        >
          <div className="glass-panel" style={{ padding: '24px' }}>
            <h4 style={{ margin: '0 0 8px 0', color: 'var(--text-secondary)' }}>
              {t('artist_dash_total_reach')}
            </h4>
            <div style={{ fontSize: '2.5rem', fontWeight: 700, color: 'var(--accent)' }}>
              {analytics.total_tickets || 0}
            </div>
            <p style={{ margin: 0, fontSize: '0.9rem', color: 'var(--text-secondary)' }}>
              {t('artist_dash_total_tickets')}
            </p>
          </div>

          <div className="glass-panel" style={{ padding: '24px' }}>
            <h4 style={{ margin: '0 0 16px 0', color: 'var(--text-secondary)' }}>
              {t('artist_dash_top_demographics')}
            </h4>
