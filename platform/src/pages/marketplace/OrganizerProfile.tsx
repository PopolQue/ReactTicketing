  ...
  if (loading)
    return (
      <div style={{ padding: '60px', textAlign: 'center' }}>
        {t('marketplace.organizerProfile.loading')}
      </div>
    );
  if (!organizer)
    return (
      <div style={{ padding: '60px', textAlign: 'center' }}>
        {t('marketplace.organizerProfile.notFound')}
      </div>
    );
        ...
        >
          ...
          <div style={{ flexGrow: 1 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
              <FollowButton entityId={id!} entityType="organizer" className="btn-primary" />
            </div>
          </div>
        </div>

        <h2 style={{ marginBottom: '24px' }}>
          {t('marketplace.organizerProfile.eventsBy')}
          {organizer.company_name}
        </h2>
        {events.length === 0 ? (
          <p style={{ color: 'var(--text-secondary)' }}>
            {t('marketplace.organizerProfile.noUpcomingEvents')}
          </p>
        ) : (
          <div
            style={{
