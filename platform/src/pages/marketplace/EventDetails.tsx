...
const EventMapDisplay = React.lazy(() =>
  import('../../components/EventMapDisplay').then((m) => ({ default: m.EventMapDisplay }))
);
...
export default function EventDetails() {
  ...
  const [checkoutResolve, setCheckoutResolve] = useState<
    ((result: 'confirmed' | 'cancelled') => void) | null
  >(null);
  ...
  const handleTicketIssued = (ticket: any, assets: any) => {
    ...
    console.log(
      `[Email Handler] Ticket issued: ${ticket.id}. Sending PNG blob to buyer ${ticket.buyerEmail}...`,
      assets
    );
    ...
    if (pixels) {
      if (pixels.metaPixelId)
        console.log(`[Meta Pixel] Firing Purchase event to ID: ${pixels.metaPixelId}`);
      if (pixels.googleAnalyticsId)
        console.log(`[GA4] Firing purchase event to ID: ${pixels.googleAnalyticsId}`);
      if (pixels.tiktokPixelId)
        console.log(`[TikTok] Firing CompletePayment event to ID: ${pixels.tiktokPixelId}`);
    }
  };
  ...
  if (loading)
    return (
      <div style={{ padding: '60px', textAlign: 'center' }}>
        {t('marketplace.eventDetails.loading')}
      </div>
    );
  if (!event)
    return (
      <div style={{ padding: '60px', textAlign: 'center' }}>
        {t('marketplace.eventDetails.notFound')}
      </div>
    );
      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            ...
            >
              {t('marketplace.eventDetails.presentedBy')}
              {event.organizers?.name || t('marketplace.eventDetails.independentOrganizer')}
            </p>
            <FollowButton entityId={event.id} entityType="event" />
            {event.organizer_id && (
              <FollowButton entityId={event.organizer_id} entityType="organizer" />
            )}
          </div>
          ...
          {(event.event_geometry || event.venues?.venue_geometry) && (
            ...
            >
              <EventMapDisplay
                geometry={
                  event.event_geometry && Object.keys(event.event_geometry).length > 0
                    ? event.event_geometry
                    : event.venues?.venue_geometry
                }
                position={[event.latitude || 52.52, event.longitude || 13.4]}
          ...
          {event.is_external ? (
            ...
            >
              <h3 style={{ marginBottom: '16px' }}>
                {t('marketplace.eventDetails.ticketsAvailableExternally')}
              </h3>
              {event.external_url ? (
                <a
                  ...
                  onClick={() =>
                    posthog?.capture('external_ticket_link_clicked', {
                      event_id: event.id,
                      event_name: event.name,
            ...
            <div
              className="react-ticket-container"
              style={{ '--rt-bg': customBgColor, '--rt-accent': customAccentColor } as any}
            >
              <React.Suspense
                fallback={
                  <div style={{ padding: '24px', textAlign: 'center' }}>
                    {t('marketplace.eventDetails.loadingTickets')}
                  </div>
                }
              >
