...
export default function Discover() {
  ...
  const getPlaceholder = () => {
    switch (activeTab) {
      case 'events':
        return t('marketplace.discover.placeholderEvents');
      case 'artists':
        return t('marketplace.discover.placeholderArtists');
      case 'venues':
        return t('marketplace.discover.placeholderVenues');
      case 'organizers':
        return t('marketplace.discover.placeholderOrganizers');
    }
  };
        ...
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
          ...
          <div style={{ maxWidth: '600px', margin: '0 auto', display: 'flex' }}>
            <input
              ...
              onBlur={() => {
                if (searchQuery.trim())
                  posthog?.capture('event_searched', { query: searchQuery.trim(), tab: activeTab });
              }}
            ...
            {viewMode === 'map' && activeTab === 'events' ? (
              ...
              >
                <p style={{ color: 'var(--text-secondary)', fontSize: '1.2rem' }}>
                  {t('marketplace.discover.noResults')}
                </p>
                <button
                  onClick={() => {
                    setSearchQuery('');
                ...
                >
                  {t('marketplace.discover.clearFilters')}
                </button>
              </div>
            ) : activeTab === 'events' ? (
              <>
                {[
                  { id: 'clubnight', title: t('marketplace.discover.clubnights') },
                  { id: 'concert', title: t('marketplace.discover.concerts') },
                  { id: 'festival', title: t('marketplace.discover.festivals') },
                  { id: 'workshop', title: t('marketplace.discover.workshops') },
                  { id: 'other', title: t('marketplace.discover.moreEvents') },
                ].map((cat) => {
                  const categoryEvents = filteredResults.filter(
                    (e: any) => (e.category || 'other') === cat.id
                  );
              ...
              >
                {filteredResults.map((item: any) => {
                  ...
                  if (activeTab === 'organizers')
                    return <OrganizerCard key={item.id} organizer={item} />;
                  return null;
                })}
              </div>
            )}
