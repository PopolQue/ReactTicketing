...
export class AuthService {
    ...
    eventSettings;
    _keyPromise = null;
    constructor(adapter, eventSettings) {
    ...
    async verifyAdminKey(passphrase) {
        ...
        if (providedHashBytes.length !== hashToCompare.length)
            return false;
    ...
    async hashWithSalt(passphrase, salt, iterations) {
        const enc = new TextEncoder();
    ...
    async assertScanSession(token, eventId) {
    ...
    async loginScanAccount(eventId, username, pin) {
