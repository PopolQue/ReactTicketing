...
export class TicketService {
  ...
  async prepareTickets(order: any): Promise<IssuedTicket[]> {
    ...
    for (const item of order.items) {
      ...
      if (ticketType && ticketType.capacity !== undefined) {
        const currentSold =
          ticketType.soldCount !== undefined
    ...
    for (const item of order.items) {
      const ticketType = (await this.adapter.getTicketTypes(order.eventId)).find(
      ...
      for (let i = 0; i < item.quantity; i++) {
        ...
        const ticket: IssuedTicket = {
          ...
          orderId: order.id,
          personalization: item.personalizations[i] || {
            name: 'Unknown',
            surname: 'Unknown',
            country: 'Unknown',
            city: 'Unknown',
  ...
  async deliverTicket(ticketId: string): Promise<void> {
    ...
    const hmac = btoa(Array.from(hmacArray, (b) => String.fromCharCode(b)).join(''))
      .replace(/=/g, '')
      .replace(/\+/g, '-')
      .replace(/\//g, '_');
  ...
  async transferTicket(
    ticketId: string,
    toEmail: string,
    newPersonalization: import('reactticket-core/types/ticket.types').TicketPersonalization
  ): Promise<void> {
    const ticket = await this.adapter.getTicket(ticketId);
    if (!ticket) throw new Error('Ticket not found');
