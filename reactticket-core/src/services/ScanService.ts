...
export class ScanService {
  ...
  async validateTicket(payload: string, session: ScanSession, eventId: string): Promise<ScanEvent> {
    ...
    try {
      ...
      if (this.adapter.validateTicketRpc) {
        const rpcResult = await this.adapter.validateTicketRpc(
          ticketId,
          session.accountId,
          session.token,
          new Date()
  ...
  async getAnalytics(eventId: string): Promise<AnalyticsSummary> {
    ...
    scans.forEach((s) => {
      if (!summary.scansPerAccount[s.scannedByAccountId]) {
        summary.scansPerAccount[s.scannedByAccountId] = {
          username: s.scannedByAccountName,
          count: 0,
        };
      }
  ...
  private async verifyQRLocally(
    ...
    return crypto.subtle.verify(
      ...
      key,
      signatureBuffer,
      enc.encode(`${prefix}.${eventId}.${ticketId}`)
    );
  }
